#!/usr/bin/env python3
"""Verify Stage 5.12e fixed-type functional streams across independent controls."""
...
def fail(message: str) -> None:
    raise SystemExit(f"stage5.12e functional-stream verification failed: {message}")
...
def main() -> None:
    build = subprocess.run(
        [sys.executable, str(ROOT / "stage5" / "selfhost" / "build_functional_stream.py"), "--check"],
        cwd=ROOT,
        text=True,
        capture_output=True,
        check=False,
    ...
    for item in items:
        ...
        for index, case in enumerate(item.get("tests", [])):
            ...
            if applied_type != "N":
                fail(f"{case_id}: fully applied type {applied_type!r} != 'N'")
    ...
    print("stage5.12e functional stream candidate: verified")
