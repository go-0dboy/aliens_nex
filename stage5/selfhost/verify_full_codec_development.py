#!/usr/bin/env python3
"""Execute Stage 5.12 full codec candidate v0.1 on development evidence only.

IMPORTANT: this verifier intentionally does not open the preregistered holdout.
After the first successful/failed execution of this script against candidate
v0.1, any algorithmic/candidate-wire change requires candidate v0.2 and a new
holdout under the frozen anti-tuning contract.
"""
...
def fail(message: str) -> None:
    raise SystemExit(f"stage5.12 full codec v0.1 development failed: {message}")
...
def status_projection(call):
    return projected_call(call, codec_status(__import__("build_foundation").v("r")))


def length_projection(call):
    return projected_call(call, codec_length(__import__("build_foundation").v("r")))


def stream_matches_projection(call, expected: list[int], sentinel: int):
    # Build one NEX Nat-valued assertion that forces every admitted stream item
    # and exactly one out-of-range sentinel. eq returns 1 for equality.
...
def finite_payload(result_call):
    # Rebuild the successful payload as the exact Finite* pair consumed by the
    # opposite codec. This does not inspect it on the host.
    from build_foundation import v
...
def main() -> None:
    ...
    if artifact.get("version") != "0.1" or artifact.get("status") != "stage5.12-development-candidate":
        fail("candidate identity/status drifted")
    if artifact.get("holdout_status") != "preregistered_unexecuted":
        fail("candidate artifact no longer says holdout preregistered_unexecuted")
    if development.get("classification") != "development" or development.get("status") != "frozen_before_full_codec_candidate_v0.1":
    ...
    for case in development.get("valid_terms", []):
        ...
        add_wrapper(
            f"decode:{cid}:stream",
            stream_matches_projection(decode_call, tokens, 0),
            1,
        )
        ...
        add_wrapper(
            f"encode:{cid}:stream",
            stream_matches_projection(encode_call, bit_values, 2),
            1,
        )

        # Law 1: decodeTerm(encodeTerm(term)) == term. The successful encoder
        # payload is passed directly as NEX data; the host does not materialize it.
        encoded_payload = finite_payload(encode_call)
        law1_call = app(decode, encoded_payload)
        ...
        add_wrapper(
            f"law1:{cid}:stream",
            stream_matches_projection(law1_call, tokens, 0),
            1,
        )

        # Law 2: encodeTerm(decodeTerm(bits)) == canonical(bits).
        decoded_payload = finite_payload(decode_call)
        law2_call = app(encode, decoded_payload)
        ...
        add_wrapper(
            f"law2:{cid}:stream",
            stream_matches_projection(law2_call, bit_values, 2),
            1,
        )
    ...
    print("stage5.12 full codec v0.1 development: verified")
