...
After Stages 0–5 were closed, the project re-audited its design rationale, empirical conclusions, receiver-assumption model, and novelty claims against the broader literature. The re-audit found no defect that invalidates NEX-1 v0.1 wire, static, or dynamic semantics, but it did find several places where the research narrative was too strong or where prior work had been omitted.
...
The project has not proved global minimality under program size, specification size, bootstrap size, or total transmitted information. Public research claims and the dissertation therefore use **compact** or **small experimental core** rather than presenting NEX as a proven minimal language.

Historical occurrences of `minimal` in the v0.1 specification are interpreted as a design aspiration meaning deliberately small, not as a theorem of optimality.
...
- total `S+B+P` must be measured rather than inferred from basis size alone.

### 3. Make total-cost claims lengths of concrete transmitted objects

`C = S + B + P` remains a useful conceptual ledger, but it is not an unconditional machine-free information scalar.
...
The Stage 4 value `226151 -> 2484` is a reduction in the project-defined Go evaluator **transition counter** (entries into the instrumented evaluation path), not a claim of 98.90% reduction in wall-clock time, CPU instructions, allocations, memory, or bootstrap complexity.
...
### 8. Correct the novelty/related-work claim

Formal languages for interstellar communication predate NEX. The dissertation must discuss at least:

- Freudenthal's Lincos [SRC-0020];
- CosmicOS and its explicit use of programs/simulations [SRC-0021];
- Lingua Cosmica work using constructive type theory [SRC-0022].

NEX therefore does **not** claim novelty for "a language/programming system for aliens" or for introducing types into interstellar-message research.

The narrower candidate contribution is the combination of:

- compact typed binary Core;
- exact program-bit accounting;
- explicit receiver-conditioned specification/bootstrap ledger;
- frozen empirical comparisons including negative results;
- blind independent reconstruction followed by differential conformance.
...
### 10. Strengthen the next evidence stage before redesigning v0.1

Before treating NEX as formally established or empirically representative, prioritize:
...
3. bounded exhaustive enumeration of small well-scoped/well-typed terms across independent implementations;
4. hold-out and externally specified workload families in addition to the design corpus;
5. a real dependency-closed bootstrap artifact under `A1(R)` or `A2(U)`.

These are higher-value research tasks than immediately changing the NEX-1 v0.1 wire format.

## Consequences

- Stages 0–5 remain complete; their exact recorded measurements remain valid within their documented conditions.
...
- The dissertation and README must describe NEX as compact/experimental rather than proven minimal.
- The project novelty claim becomes narrower and better grounded in prior art.

## Evidence

This ADR is based on the post-Stage-5 audit recorded in:

- `docs/RESEARCH-AUDIT-2026-09-19.md`;
- `docs/RESEARCH-AUDIT-2026-09-19.ru.md`.

New external evidence is registered as SRC-0018 through SRC-0027 in `docs/SOURCES.md`.
