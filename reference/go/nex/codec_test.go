...
func TestURejectsTruncatedAndLimits(t *testing.T) {
	...
	bits, _ = ParseBits("0001000") // U(7), m has 4 bits
...
func TestDecodeOneReportsConsumedBits(t *testing.T) {
	bits, _ := ParseBits("010011") // identity + one extra bit
	term, consumed, err := DecodeOne(bits, DefaultDecodeLimits)
	if err != nil {
		t.Fatal(err)
	}
	if !EqualTerm(term, Lam(Var(NaturalUint64(0)))) {
		t.Fatal("unexpected term")
	}
	if consumed != 5 {
...
func TestInvalidBitRejected(t *testing.T) {
	...
	if !errors.Is(err, ErrInvalidBit) {
		t.Fatalf("got %v want ErrInvalidBit", err)
	}
}
...
func TestEncodeRejectsInvalidShape(t *testing.T) {
	...
	if !errors.Is(err, ErrInvalidTerm) {
		t.Fatalf("got %v want ErrInvalidTerm", err)
	}
}
...
func FuzzTermRoundTrip(f *testing.F) {
	for _, seed := range [][]byte{{0}, {1, 2, 3}, {2, 4, 8, 16}, {3, 1, 4, 1, 5, 9}} {
		f.Add(seed)
	}
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		if !EqualTerm(got, term) {
			t.Fatal("round-trip term mismatch")
		}
	})
}
