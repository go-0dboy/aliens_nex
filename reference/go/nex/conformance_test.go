...
func loadConformance(t *testing.T) conformanceFile {
	...
	if err != nil { t.Fatalf("read conformance file: %v", err) }
	var c conformanceFile
	if err := json.Unmarshal(data, &c); err != nil { t.Fatalf("parse conformance file: %v", err) }
	return c
}
...
func naturalFromDecimal(t *testing.T, s string) *big.Int {
	...
	if !ok || n.Sign() < 0 { t.Fatalf("invalid natural %q", s) }
	return n
}
...
func termFromJSON(t *testing.T, j jsonTerm) *Term {
	...
	case "Lam":
		if j.A == nil { t.Fatal("Lam missing a") }
		return Lam(termFromJSON(t, *j.A))
	case "App":
		if j.A == nil || j.B == nil { t.Fatal("App missing child") }
		return App(termFromJSON(t, *j.A), termFromJSON(t, *j.B))
	case "Let":
		if j.A == nil || j.B == nil { t.Fatal("Let missing child") }
...
func TestConformanceIntegerVectors(t *testing.T) {
	...
	for _, tc := range c.IntegerVectors {
		t.Run(tc.Name, func(t *testing.T) {
			...
			if encoded.String() != tc.Bits { t.Fatalf("encoded=%s want=%s", encoded.String(), tc.Bits) }
...
func TestConformanceTermVectors(t *testing.T) {
	...
	for _, tc := range c.TermVectors {
		t.Run(tc.Name, func(t *testing.T) {
			...
			if err != nil { t.Fatal(err) }
			if !EqualTerm(decoded, term) { t.Fatal("decoded term differs") }
		})
	}
}
...
func TestConformanceInvalidExactVectors(t *testing.T) {
	...
	for _, tc := range c.InvalidExactVectors {
		t.Run(tc.Name, func(t *testing.T) {
			...
			switch tc.Error {
			case "truncated":
				if !errors.Is(err, ErrTruncated) { t.Fatalf("got %v want truncated", err) }
			case "trailing":
				if !errors.Is(err, ErrTrailingBits) { t.Fatalf("got %v want trailing", err) }
			default:
